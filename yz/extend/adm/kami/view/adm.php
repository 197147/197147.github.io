...
?>
						...
						<table class="table table-centered table-striped dt-responsive nowrap w-100">
							<thead>
								<tr>
									...
									<th style="width: 150px;"><center><span class="badge badge-light-lighten">增减时间</span></center></th>
									...
									<th style="width: 200px;"><center><span class="badge badge-light-lighten">到期时间</span></center></th>
									<th style="width: 150px;"><center><span class="badge badge-light-lighten">剩余时间</span></center></th>
									...
									<th style="width: 150px;"><center><span class="badge badge-light-lighten">机器码</span></center></th>
							...
							<tbody>
								...
								<tr>
									...
									<td>
										...
										<td>
										<?php
												echo "<a href=\"javascript:void(0);\" onclick=\"km_time_id(".$rows['id'].")\"> <span class=\"badge badge-light\" data-toggle=\"modal\" data-target=\"#km_time-modal\"> 点击修改</span></a>";
										?>
									</td>
									...
									<td>
										<center>
                                     <?php if($rows['type']=='fen'):?><span class="badge badge-primary">积分卡[<?php echo $rows['amount']; ?>]
                                    <?php elseif($rows['type']=='TK'):?><span class="badge badge-dark-lighten">卡密天卡
                                    <?php elseif($rows['type']=='ZK'):?><span class="badge badge-dark-lighten">卡密周卡
                                    <?php elseif($rows['type']=='YK'):?><span class="badge badge-dark-lighten">卡密月卡
                                    <?php elseif($rows['type']=='BNK'):?><span class="badge badge-dark-lighten">卡密半年卡
                                    <?php elseif($rows['type']=='NK'):?><span class="badge badge-dark-lighten">卡密年卡
                                    <?php elseif($rows['type']=='YJK'):?><span class="badge badge-dark-lighten">卡密永久卡
                                    <?php else: ?><span class="badge badge-danger-lighten">会员卡[ <?php if($rows['amount'] == 999999999){echo '永久卡';}else{ echo $rows['amount'].'天';} ?> ]<?php endif; ?> </span>
                                       </center>
									</td>
									
									<td>
										<center>
										<?php if($rows['user']):?><span class="badge badge-success">
									...
									<td>
										...
										<?php else: ?><span class="badge badge-primary"><?php echo date("Y-m-d H:i",$rows['use_time']); endif; ?></span>
										</center>
									</td>
									<td>
										<center>
										<?php if($rows['end_time']==0):?><span class="badge badge-info-lighten">未使用
										<?php else: ?><span class="badge badge-primary"><?php echo date("Y-m-d H:i",$rows['end_time']); endif; ?></span>
										</center>
									</td>
									
									<td>
										<center>
								 <?php if($rows['end_time']-time()< 0 && $rows['use_time'] >0):?><span class="badge badge-danger">已到期 
                                     <?php elseif($rows['end_time']-$rows['use_time']):?><span class="badge badge-primary"><?php echo (round(($rows['end_time']-time())/86400)).'天';?> 
                                    <?php else:?><span class="badge badge-success-lighten">未使用
                                    <?php endif; ?>
                                    </span>
									...
									<td>
										...
										<td>
										<center>
										<?php if($rows['codestate']=='y'):?><a href="javascript:void(0);" onclick="edit_codestate(<?php echo $rows['id']; ?>,'<?php echo $rows['codestate'];?>')"><span id="codestate_<?php echo $rows['id']; ?>" class="badge badge-danger">	<?php echo $rows['uuid'];?>
										<?php else:?><span  class="badge badge-success">未绑定
										<?php endif; ?></span></a>
										</center>
									</td>
	...
	<!--增减时间-->
		<div id="km_time-modal" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
		<div class="modal-dialog modal-dialog-centered">
			<div class="modal-content">
				<div class="modal-body">
					<div class="text-center mt-2 mb-4"></div>
					<form class="pl-3 pr-3" action="" name="note_log" id="note_log" method="post">
						<div class="form-group">
							<input class="form-control" type="number" style="display:none" id="kid" name="kid" value="" ><!--style="display:none" --> 
							<label for="username">修改卡密时间</label>
							<input class="form-control" type="number" id="km_time" name="km_time" required="" placeholder="增加或减少天数">
						</div>
						<div class="form-group text-center">
							<button class="btn btn-primary" type="submit" name="submit_km_time" id="submit_km_time" value="确认">确认增减</button>
						</div>
					</form>
				</div>
			</div><!-- /.modal-content -->
	...
	<script>
		...
		////增减时间
			...
			$('#submit_km_time').click(function() {
			let t = window.jQuery;
			var km_time = $("input[name='km_time']").val();
			var kid = $("input[name='kid']").val();
			document.getElementById('submit_km_time').innerHTML="<span class=\"spinner-border spinner-border-sm mr-1\" role=\"status\" aria-hidden=\"true\"></span>正在修改卡密时间";
			document.getElementById('submit_km_time').disabled=true;
			
			$.ajax({
				cache: false,
				type: "POST",//请求的方式
				url : "ajax.php?act=kami_kmtime",//请求的文件名
				data : {
					km_time:km_time,
					kid:kid
				},
				...
				success : function(data) {
					console.log(data);
					document.getElementById('submit_km_time').disabled=false;
					document.getElementById('submit_km_time').innerHTML="确认增减";
					if(data.code == 200){
						t.NotificationApp.send("成功",data.msg,"top-center","rgba(0,0,0,0.2)","success");
						window.setTimeout("window.location='"+window.location.href+"'",1000);
					}else{
		...
		function km_time_id(i,s) {
			var kid=document.getElementById("kid");
			var km_time=document.getElementById("km_time");
			kid.value= i;
			km_time.value= s;
		}
		...
		function edit_state(id,state) {
			...
			function edit_codestate(id,codestate) {
			let t = window.jQuery;
			var badge = document.getElementById("codestate_"+id).className;
			if(badge == 'badge badge-danger'){
				codestate='y'
				document.getElementById("codestate_"+id).className = "badge badge-danger";
				//document.getElementById("codestate_"+id).innerHTML="已绑定机器码";
			}
			...
			$.ajax({
				cache: false,
				type: "POST",//请求的方式
				url : "ajax.php?act=kami_codestate",//请求的文件名
				data : {id:id,codestate:codestate},
				dataType : 'json',
				success : function(data) {
					if(data.code == 200){
						t.NotificationApp.send("解绑成功",data.msg,"top-center","rgba(0,0,0,0.2)","success")
					}else{
						t.NotificationApp.send("解绑失败",data.msg,"top-center","rgba(0,0,0,0.2)","error")
					}
				}
			});
			return false;//重要语句：如果是像a链接那种有href属性注册的点击事件，可以阻止它跳转。
