...
?>						
					...
					<form class="needs-validation" novalidate  action="" method="post">
						<div class="form-row">
							<div class="form-group col-md-6">
								...
								<select class="form-control" name="add_type" id="add_type" onchange="type_change(this.value)">
									...
									<option value="TK" >卡密天卡</option><!-- 卡密登录修改-->
									<option value="ZK" >卡密周卡</option> <!-- 卡密登录修改-->
									<option value="YK" >卡密月卡</option> <!-- 卡密登录修改-->
									<option value="BNK" >卡密半年卡</option> <!-- 卡密登录修改-->
									<option value="NK" >卡密年卡</option> <!-- 卡密登录修改-->
									<option value="YJK" >卡密永久卡</option><!-- 卡密登录修改-->
							...
							<div class="form-group col-md-6">
								<label class="col-form-label" id="amount_name">&nbsp;会员天数 *(卡密类型为会员可用)</label>
								<div class="input-group">
									<input type="number" id="add_amount" name="add_amount" class="form-control" placeholder="会员天数，永久卡10个9，仅支持数字" 
									value="" required>
						...
						<div class="form-row">
							<div class="form-group col-md-12">
								<label class="col-form-label">卡密前缀</label>
								<input type="text" class="form-control" id="add_prefix" name="add_prefix" placeholder="卡密前缀" value="" required>
							</div>
						</div>        
	...
	<script>
		$('#add_submit').click(function() {
			...
			var add_prefix = $("input[name='add_prefix']").val();
		...
		function type_change(i) {
			...
			if(i=='vip'){
				...
				$("#add_amount").val("0");
                $("#add_amount").attr("disabled",false);//卡密登录修改
			}else if(i=='fen'){
				document.getElementById('amount_name').innerHTML="&nbsp;积分数 *";
				document.getElementById('add_amount').setAttribute("placeholder","积分数，仅支持数字");
			...
			//天卡
			    ...
			    document.getElementById('amount_name').innerHTML="&nbsp;卡密天数 *";
				document.getElementById('amount_a').innerHTML="天";
				$("#add_amount").val("1");//卡密登录修改
				$("#add_amount").attr("disabled",true);//卡密登录修改
			}
			//周卡
			else if(i=='ZK'){
			    document.getElementById('amount_name').innerHTML="&nbsp;卡密天数 *";
				document.getElementById('amount_a').innerHTML="天";
				$("#add_amount").val("7");//卡密登录修改
				$("#add_amount").attr("disabled",true);//卡密登录修改
			}
			//月卡
			else if(i=='YK'){
		   ...
		   //永久卡
			     ...
			     document.getElementById('amount_name').innerHTML="&nbsp;卡密天数 *";
				document.getElementById('amount_a').innerHTML="天";
				$("#add_amount").val("9999999999");//卡密登录修改
				$("#add_amount").attr("disabled",true);//卡密登录修改
			}
