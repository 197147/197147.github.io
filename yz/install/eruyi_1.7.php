...
"CREATE TABLE IF NOT EXISTS `{$db_pre}app` (
  ...
  `mi_sign` enum('y','n') DEFAULT 'n' COMMENT '是否签名',
  ...
  `kmlogon_check_in` enum('y','n') DEFAULT 'y' COMMENT '卡密登录设备验证',
  `kmlogon_check_t` int(10) DEFAULT '24' COMMENT '换绑扣除时间间隔',
  `kmlogon_num` int(10) DEFAULT '1' COMMENT '卡密多设备登录',
  PRIMARY KEY (`id`),
...
"CREATE TABLE IF NOT EXISTS `{$db_pre}kami` (
  ...
  `type` enum('vip','fen','TK','ZK','YK','BNK','NK','YJK') DEFAULT 'vip' COMMENT '卡密类型',
  ...
  `state` enum('y','n') DEFAULT 'y' COMMENT '状态',
  `prefix` text COMMENT '卡密前缀',
  `uuid` varchar(255) DEFAULT NULL COMMENT '机器码',
  `codestate` enum('y','n') DEFAULT 'n' COMMENT '机器码绑定状态',
  PRIMARY KEY (`id`),
